...
const tools = {
  ...
  impact_get_framework: {
    ...
    execute: (args: { focus_phase?: string }) => {
...
`;
      ...
      return phases[phase as keyof typeof phases] || 'Phase not found. Use: identify, map, pinpoint, anchor, craft, translate, or all';
    }
  },
...
`;
  ...
  impact_map_alternatives: {
    ...
    execute: (args: {
      ...
      let marketContext = {
        typical_leaders: [] as string[],
        common_complaints: [] as string[],
        differentiation_axes: [] as string[]
      };
  ...
  impact_pinpoint_value: {
    ...
    execute: (args: {
      ...
      if (metrics) {
        ...
        if (percentMatch) quantifiedResults.primary = percentMatch[1] + '%';
        if (multiplierMatch) quantifiedResults.secondary = multiplierMatch[1] + 'x';
        if (timeMatch) quantifiedResults.time = timeMatch[0];
      }
...
**Issue**: ${
  phase === 'identify' ? 'Champion identification needs more specificity. Who exactly is your buyer?' :
